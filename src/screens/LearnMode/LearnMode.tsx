...
export const LearnMode = ({ onClose, isOpen = true, onLevelSelect, selectedLevel = "learn" }: LearnModeProps): JSX.Element => {
  ...
  // Render via portal to escape any clipping/stacking contexts created by the plugin layout
  // (e.g. overflow-hidden / transformed ancestors in WKWebView).
  return createPortal(
    <div
      style={{
        position: 'fixed',
        ...
        >
          ...
          <div className="flex flex-col h-[195px] items-start gap-[11px] relative self-stretch w-full">
            {proficiencyOptions.map((option) => (
              <div key={option.id} className="h-[49px] relative self-stretch w-full">
                <label
