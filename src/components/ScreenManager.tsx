...
export default function ScreenManager() {
  ...
  const BUILD_TAG = "fix-host-restart-resume-rules-1";
  ...
  const computeAutoStartStep = (): Step => {
    ...
    // Host boot detection (Ableton/DAW restart):
    // Requirement:
    // - OS reboot => SignIn (handled above by osBootId/lastOsBootId)
    // - Host restart WITHOUT OS reboot => force "Your Projects" (NOT chat), even if last screen was chat.
    // - Same host session (plugin window reopened) => resume last screen via LAST_STEP_KEY.
    //
    // We compare a runtime boot id injected by C++ to the last seen boot id persisted in PropertiesFile.
    // NOTE: this must be stable for the host process; if it changes every WebView reload, the C++ side must be fixed.
